...
function createSimilarPackets(basePacket, count, idOffset) {
    let resultPacket = JSON.parse(JSON.stringify(basePacket));

    if (
        resultPacket.title.includes("%1") &&
        resultPacket.title.includes("%2")
    ) {
        const mpptIndex = Math.floor(count / 2);
        const channelIndex = count % 2;
        resultPacket.title = resultPacket.title
            .replace("%1", mpptIndex)
            .replace("%2", channelIndex);
    } else {
        resultPacket.title += count;
    }

    resultPacket.fields.forEach((field) => {
        field.id =
            "0x" +
            (parseInt(field.id, 16) + idOffset).toString(16).toUpperCase();
    });

    return resultPacket;
}

menuModel.push(createSimilarPackets(motorBase, 0, 0x00));
menuModel.push(createSimilarPackets(motorBase, 1, 0x20));

for (let i = 0; i < 8; i++) {
    menuModel.push(createSimilarPackets(mpptBase, i, i));
}
